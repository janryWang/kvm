...
define("$do", function () {
	...
	var KEYWORDS = (function () {
		var cmd_name, cmds_name, keywords, _i, _len, _ref;
		keywords = {};
	...
	kvm.merge(Do.prototype, {
		_collect: function _collect(obj) {
			var name, res;
			res = {
				operators: {},
				filters: {},
		...
		_filters: {
			...
			$icontains: function $icontains(a, b) {
		...
		_commands: {
			$set: function $set(ref, collected) {
				var prop_name, prop_val, _ref, _results;
			...
			$remove: function $remove(ref, collected) {
				var prop_name, prop_val, _ref, _results;
				_ref = collected.props;
				_results = [];
				for (prop_name in _ref) {
			...
			$slice: function $slice(ref, collected) {
				var prop_name, prop_val, _ref, _results;
				_ref = collected.props;
				_results = [];
				for (prop_name in _ref) {
			...
			$push: function $push(ref, collected) {
				var prop_name, prop_val, _ref, _results;
				_ref = collected.props;
				_results = [];
				for (prop_name in _ref) {
			...
			$concat: function $concat(ref, collected) {
				var prop_name, prop_val, _ref, _results;
				_ref = collected.props;
				_results = [];
				for (prop_name in _ref) {
			...
			$pop: function $pop(ref, collected) {
				var prop_name, prop_val, _ref, _results;
				_ref = collected.props;
				_results = [];
				for (prop_name in _ref) {
			...
			$unshift: function $unshift(ref, collected) {
				var prop_name, prop_val, _ref, _results;
				_ref = collected.props;
				_results = [];
				for (prop_name in _ref) {
			...
			$merge: function $merge(ref, collected) {
				return kvm.merge(ref, collected.props);
			},
			$deep_merge: function $deep_merge(ref, collected) {
				return kvm.merge(true, ref, collected.props);
			},
			$clone: function $clone(ref, collected) {
				var blackList, callback, filter, isDeep, res, whiteList;
				...
				if (!kvm.isEmpty(whiteList)) {
					res = kvm.copy(isDeep, ref, function (item, name, path) {
						var filter_res;
			...
			$find: function $find(ref, collected) {
				var callback, condition_length, res, _this;
				_this = this;
				callback = collected.commons.$callback;
					...
					function (ref_item) {
						...
						if (!kvm.isEmpty(ref_item)) {
							...
							function (filter_body, filter_name) {
								...
								kvm.forEach(filter_body, function (item, key) {
								...
								if (isAnd2) {
									if (ok2 >= kvm.getKeyLength(filter_body)) {
										//入内层筛选条件全部成立，组合条件成立
										ok1++;
									}
								} else {
		...
		_exec: function _exec(ref, cmd_obj) {
			var cmd_body, cmd_name, _results;
			_results = [];
...
//# sourceMappingURL=kvm-compiled.js.map